...
struct callerase<R(Args...)> {
    template <typename Callable>
    constexpr callerase(Callable callable) : ptr{std::make_unique<implementation<Callable>>(callable)} {}
  ...
  private:
    ...
    struct implementation final : interface {
        constexpr explicit(true) implementation(Callable callable) : callable{callable} {}
        constexpr auto operator()(Args... args) -> R { return callable(args...); }
...
// This overload participates in overload resolution only if &Callable::operator() is
// well-formed when treated as an unevaluated operand and
// decltype(&Callable::operator()) is of the form R(G::*)(A...) (optionally
// cv-qualified, optionally noexcept, optionally lvalue reference qualified).
// The deduced type is std::function<R(A...)>.
template <typename Callable>
callerase(Callable) -> callerase<callerase_type_t<decltype(&Callable::operator())>>;
