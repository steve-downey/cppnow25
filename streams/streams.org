...
#+end_src
*** Construction
    /left/   = \lambda a l r. l a

    /right/  = \lambda b l r. r b
...
* The Pattern(s)
** Non-recursive Types
For a type \Tau  with  constructors /A/, /B/, /C/, ... using types a_{1}, a_{2},a_{3}, ...
...
- /A/ \equiv  \lambda a_{1} a_{2} f_{1} f_{2} f_{3} . f_{1} a_{1} a_{2}
- /B/ \equiv  \lambda a_{2} a_{3} f_{1} f_{2} f_{3} . f_{2} a_{2} a_{3}
- /C/ \equiv  \lambda a_{4} f_{1} f_{2} f_{3} . f_{3} a_{4}
**** A function taking a /\Tau/
- f (A x y) = body_{A}
- f (B y z) = body_{B}
- f (C w)   = body_{C}
**** Encode the function
/f/  \equiv \lambda t . (\lambda a_{1} a_{2} . body_{A}) (\lambda a_{2} a_{3} . body_{B}) (\lambda a_{4} . body_{C})
